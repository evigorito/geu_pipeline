#' ---
#' title: Geuvardis eQTL reference bias correction2 and corrected likelihood with same prior with/without GTs
#' author: Elena
#' output:
#'    pdf_document:
#'     toc: true
...
## source=c("GT","RNAp054")
## rbias=c("norefbias", "refbias")

## btrec_dir  <- "/mrc-bsu/scratch/ev250/EGEUV1/quant/refbias/Btrecase/SingleCounting"
...
trec.refbias <- merge(trec, btrec$GT[rbias=="refbias" & model =="trec-ase",], by=c("Gene_id", "tag"), suffixes=c(".trec",".trecase"))
...
btrecase.plot(dt=trec.refbias[Signif != "None",] ,
              x2=c(paste0(c("log2_aFC_mean", "log2_aFC_2.5%", "log2_aFC_97.5%"), ".trecase"), "null.95"),
...
##' # Look at distance to gene by Signif in tre.refbias
colors=c(None="#999999", `trec-ase`="yellow3", trec="#0072B2", Both= "#D55E00")
ggplot(trec.refbias[Signif!="None",], aes(x=abs(gene.dist), color=Signif)) + geom_density() +
    scale_colour_manual(values=colors[levels(trec.refbias$Signif) != "None"])


ggplot(trec.refbias[Signif!="None",], aes(x=nhets.trecase, color=Signif)) + geom_density() +
    scale_colour_manual(values=colors[levels(trec.refbias$Signif) != "None"])

## plot predictions from logistic regression

## need to code sig as 0 or 1 and run regressions for Both, Trec and TrecASE relative to NONE

l <- lapply(c("trec","Both","trec-ase"), function(i) {
    tmp <- trec.refbias[, .(gene.dist, Signif)]
    tmp[ , gene.dist:=abs(gene.dist)][, y:=0][Signif==i, y:=1]
    return(tmp)
})

## predict
g.odds <- lapply(l, function(i) {
    p=predict(lrm(y ~ rcs(gene.dist), data = i), newdata=data.frame(gene.dist=seq(0, 500000, 10000))
              )} )
names(g.odds) <- c("trec","Both","trec-ase")

## convert to prob and plot

p <- data.table(sapply(g.odds, function(i) exp(i)/(1+exp(i))))

p[, dist:=seq(0, 500000, 10000)]

p.long <- melt(p, id.vars="dist")

ggplot(p.long, aes(dist/1000, value, group=variable)) +      
theme_bw() +
...
theme(axis.title = element_text(size=14),
    ...
    scale_color_manual(values=c("#F0E442", "#D55E00", "#0072B2"))
