#' ---
#' title: Benchmarking eQTL methods
#' author: Elena
#' output:
#'    pdf_document:
#'     toc: true
...
maxRhat <- 1.1
## btrecase_dir  <- "/mrc-bsu/scratch/ev250/EGEUV1/quant/refbias/Btrecase/SpikeMixV3_2/GT"
btrecase_dir <- snakemake@params[['btrecase']]



## open files and apply QC
baseqtl <- comb.files(path=btrecase_dir,
                      pattern=paste0(paste0("^",
                                            rbias,
                                            "\\.ENSG[0-9]+.*stan.summary.txt")))
baseqtl <- baseqtl[Rhat < maxRhat,]
...
baseqtl.fdr <- rbindlist(lapply(c(99,95,90,85, 80)/100, function(i){
...
}, c(dseq.l, rasqual, lm, list(baseqtl)))


common.all <- lapply(c(dseq.l, rasqual, list(rasqual.per=rasq.perm), lm, list(baseqtl=baseqtl)),
...
## make long format for common associations
## dseq.l <- rbindlist(lapply(r.fdr, function(i) {
##     tmp <- common.all$DESeq2[, null.Fdr:="yes"][p_adjust<=i, null.Fdr:="no"]
##     tmp[, Fdr:= i]
##     tmp <- tmp[, .(Gene_id, tag, log2_aFC_mean, log2_aFC_se_mean, p_adjust, null.Fdr, Fdr)]
##     return(tmp)
## }))
...
rasq.per.l <- long(rasq.perm, c("Gene_id", "tag","log2_aFC_mean","p",
                                       grep("null.fdr",names(rasq.perm), value=T)))

## restrict to common assoc
baseqtl.l <- merge(baseqtl.fdr, assoc, by=names(assoc))
...
common.all.l <- c(dseq2.l, rasq.l, list(rasq.perm=rasq.per.l), lm.l, list(baseqtl=baseqtl.l))
...
comp.geu.assoc[, Fdr.approx:=Fdr][Method=="baseqtl",
                                  Fdr.approx:=c( 0.001, 0.010, 0.050, 0.1, 0.15)][, Fdr.approx:=as.factor(Fdr.approx)]


## compare all methods with 86 inds with Fdr 0.001-0.01 (as before)
fdr.plot(dt=comp.geu.assoc[Fdr.approx %in% c( 0.001, 0.010, 0.050, 0.1) & 
                                    Method %in% c("None", "baseqtl","lm.Inds86_gclibsize", "rasq.perm"), ],
                  x="PPV",
                  y="Sensitivity",
                  lab.col="N.total",
...
fdr.plot(dt=comp.geu.assoc[Method %in% c("None", "baseqtl","lm.Inds86_gclibsize", "rasq86"), ],
                  x="PPV",
                  y="Sensitivity",
                  lab.col="N.total",
         type="associations", col="sig.geu",
...
fdr.lm.a <- fdr.plot(dt=comp.geu.assoc[Fdr.approx %in% c(0.001, 0.010, 0.050, 0.10) &
                                       Method %in% c("lm.log_counts","lm.Inds86_gclibsize" ), ],
                  x="PPV",
                  y="Sensitivity",
                  lab.col="N.total",
                  type="associations", col="sig.geu")
...
## compare ln and rasqual different Inds

## fdr.ind.lmr.a <- fdr.plot(dt=comp.geu.assoc[Fdr.approx %in% c(0.001, 0.010, 0.050, 0.10) &
##                                             Method %in% c(names(lm)[2:4], names(rasqual)), ],
##                   x="PPV",
##                   y="Sensitivity",
##                   lab.col="Fdr.approx",
##                   type="associations", col="sig.geu")
...
## fdr.lm.rasq <- fdr.plot(dt=comp.geu.assoc[Fdr %in% r.fdr &
##                                        Method %in% c("rasq86", "lm.Inds86_gclibsize"), ],
##                   x="PPV",
##                   y="Sensitivity",
##                   lab.col="N.total",
##                   type="associations", col="sig.geu")
...
comp.geu.g[, Fdr.approx:=Fdr][Method=="baseqtl",
                                  Fdr.approx:=c( 0.001, 0.010, 0.050, 0.10, 0.15)][, Fdr.approx:=as.factor(Fdr.approx)]


## compare all methods with 86 inds
fdr.plot(dt=comp.geu.g[  Method %in%  c("None", "baseqtl","lm.Inds86_gclibsize", "rasq86"), ],
                  x="PPV",
                  y="Sensitivity",
                  lab.col="N.total",
...
fdr.plot(dt=comp.geu.g[ Fdr.approx %in% c( 0.001, 0.010, 0.050, 0.10, 0.15) & Method %in%  c("None", "baseqtl","lm.Inds86_gclibsize", "rasq.perm"), ],
                  x="PPV",
                  y="Sensitivity",
                  lab.col="N.total",
         type="eGenes", col="sig.geu",
...
fdr.plot(dt=comp.geu.g[Fdr.approx %in% c(0.001, 0.010, 0.050, 0.10) &
                                       Method %in% c(names(dseq.l), "lm.Inds86_gclibsize" ), ],
                  x="PPV",
                  y="Sensitivity",
                  lab.col="N.total",
...
tpfp.geu <- egenes.f(common.all.l[c(#"DESeq2",
                                    "baseqtl","lm.log_counts", "rasq86")], assoc, sig.qtl)
...
## compare all methods with 86 inds (as before)
tpfp.geu <- egenes.f(common.all.l[c(#"DESeq2",
                                    "baseqtl","lm.log_counts", "rasq.perm")], assoc, sig.qtl)
